      ...
      begin
        ...
        open(URI.escape(url)) do |stream|
          ...
          raise(TranslateServerIsDown) if (!result || result.empty?)
	        ...
	        # r2 = result[0][0][2]
	        # [r1, r2]
	        if result[5][0][2].size == 0
	        	array_translate = result[5][1][2]
  				else
  					array_translate = result[5][0][2]
        	end
	        final_result = array_translate.collect do |s|  
    				s[0]
					end
        end
      rescue Exception => e
...
# puts translator.translate(:en, :es, "love")
